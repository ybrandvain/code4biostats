...
my_param <- base::sample(c(.2,.4,.5), size = 1, prob = c(1/4,1/4,1/2))
...
my_estimate <- tibble(flips = base::sample(c("Heads", "Tails"),
                                           prob = c(my_param, 1 - my_param),
                                           replace = TRUE,
                                           size = n_flips)) %>%
  summarise(p_heads = base::mean(flips == "Heads")) %>%
  pull()
...
n_reps <- 100000
null_samp_dist <- tibble(flips = base::sample(c("Heads", "Tails"),
...
### Find a p-value
null_samp_dist %>%
  mutate(as_or_more  = abs(p_heads - null_value ) >= abs(my_estimate- null_value))%>%
  summarise(p_value = base::mean(as_or_more))
